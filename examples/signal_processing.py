...
b.center().plot(ax=ax, fmt="b-", alpha=0.7, label="centered")
b.rectify().plot(ax=ax, fmt="g-", alpha=0.7, label="rectified (abs)")
ax.legend()
...
a.plot(ax=ax, fmt="k-", label="raw")
band_pass.plot(ax=ax, fmt="r-", alpha=0.7, label="band-pass @ 10-200Hz")
...
a.plot(ax=ax, fmt="k-", label="raw")
band_stop.plot(ax=ax, fmt="r-", alpha=0.7, label="band-stop @ 49.9-50.1Hz")
...
a.plot(ax=ax, fmt="k-", label="raw")
high_pass.plot(ax=ax, fmt="r-", alpha=0.7, label="high-pass @ 30Hz")
...
emg = (
    a.band_pass(freq=a.get_rate, order=4, cutoff=[10, 425])
    .center()
    .rectify()
...
low_pass.plot(ax=ax[0], fmt="r-", alpha=0.7, label="low-pass @ 5Hz")
...
# fft on real data
emg_without_low_pass = (
    a.band_pass(freq=a.get_rate, order=4, cutoff=[10, 425]).center().rectify()
)
...
emg_without_low_pass.plot(ax=ax[0], fmt="k-", label="raw", alpha=0.7)
...
norm = b.center(mu=np.nanmean(b[..., : int(b.get_rate)], axis=-1), axis=-1).norm()
